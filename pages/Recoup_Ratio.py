...
def load_data(url, state_name):
    ...
    columns_to_load = ['STATENAME', 'LEA_NAME', 'LCITY', 'MEMBER', 'STUTERATIO', 'District Name', 'Region', 'geometry', 'AVG(Recoup rate)']
...
alias = 'Recoup rate'
geo_data = geo_data.rename(columns={'AVG(Recoup rate)': alias})
...
st.markdown("""
<div class="legend">
    <div><div class="color-box" style="background: rgba(255, 255, 255, 0.55);"></div>Low Recoup Ratio</div>
    <div><div class="color-box" style="background: rgba(128, 128, 255, 0.55);"></div>Medium Recoup Ratio</div>
    <div><div class="color-box" style="background: rgba(0, 0, 255, 0.55);"></div>High Recoup Ratio</div>
</div>
""", unsafe_allow_html=True)
